...
SupplierOrder.getOrdersBySupplierId = async (supplierId) => {
	const result = await new Promise((resolve, reject) => {
		pool.getConnection((err, connection) => {
			...
			connection.query(
				`SELECT * FROM supplyorder WHERE supplier_id = ${supplierId}`,
...
SupplierOrder.getSupplyOrdersByDates = async (
	...
	const result = await new Promise((resolve, reject) => {
		pool.getConnection((err, connection) => {
			if (err) throw err;
			connection.query(
				`SELECT * from supplyorder where supplier_id = '${supplierId}' and date >= '${fromDateF}' and date <= '${toDateF}'`,
				(err, rows) => {
					if (err) {
						reject(err);
					} else {
...
SupplierOrder.getOrderDetailsBySupplireOrderId = async (supOrderId, res) => {
	const result = await new Promise((resolve, reject) => {
		pool.getConnection((err, connection) => {
			if (err) throw err;
			connection.query(
				`SELECT shb.*,b.buying_price,b.selling_price,i.code,i.description,i.name
		...
		const result = await new Promise((resolve, reject) => {
			pool.getConnection((err, connection) => {
				...
				connection.beginTransaction((err) => {
					connection.query(
						`select * from supplyorder so, supplyorder_has_batch shb
