...
Report.getReportStatictics = async ({ fromDate, toDate, idToken }, res) => {
	...
	const cashOrder = await Report.getCashOrderSummeryQuery(
		fromDate,
		toDate,
		userRole
	);
	const creditOrder = await Report.getCreditOrderSummeryQuery(
		fromDate,
		toDate,
		userRole
	);
	const supplyOrder = await Report.getSupplyOrderSummeryQuery(fromDate, toDate);
	const returnItem = await Report.getReturnItemSummeryQuery(
		fromDate,
		toDate,
		userRole
	);
...
Report.getReturnItemSummeryQuery = (fromDate, toDate) => {
	const result = new Promise((resolve, reject) => {
        ...
        `;
			connection.query(sql, (itemReturnSummeryErr, itemReturnSummeryResult) => {
				connection.release();
...
Report.getReportDetail = async ({ fromDate, toDate, type, idToken }, res) => {
	const authUser = await userFirebase.verifyIdToken(idToken);
	const userId = authUser;
	const user = await userModel.findByFirebaseId(userId);
	const userRole = user[0].user_role_type;
	const reportDetail = await Report.getReportDetailQuery(
		fromDate,
		toDate,
		type,
		userRole
