...
Customer.getCustomerQuery = async (customerId) => {
	const result = await new Promise((resolve, reject) => {
		pool.getConnection((err, connection) => {
			...
			connection.query(
				`SELECT * from customer where idcustomer =${customerId}`,
...
Customer.deleteCustomerQuery = async (customerId) => {
	const result = await new Promise((resolve, reject) => {
		pool.getConnection((err, connection) => {
			...
			connection.query(
				`DELETE from customer where idcustomer =${customerId}`,
...
Customer.addCustomerOrder = async (supplyOrder, res) => {
	...
	try {
                  ...
                  order by idcustomer_order desc limit 1`;
						...
						connection.query(
							...
							(customerOrderLastIdErr, customerOrderLastIdResult) => {
								...
								if (
									customerOrderLastIdResult &&
									!customerOrderLastIdResult[0]
								) {
									resolve2(1);
								} else {
									let lastNo =
										customerOrderLastIdResult[0].invoice_no.substring(3);
									resolve2(++lastNo);
								}
							}
						);
								...
								batches.forEach(async (item) => {
									await new Promise((unitLengthResolve) => {
										...
										if (
											...
											const percentageSubQty =
												((100 / item.unitLength) * item.subQty) / 100;
							...
							connection.query(
								...
								(getOrderrror, getOrderResult) => {
									...
									connection.commit(async (errCommit) => {
										...
										// connection.release();
										const customer = await Customer.getCustomerQuery(
											customerId
										);
										const response = {
											code: 200,
		...
		const customerDebit = await ReturnItem.getReturnDebitByCustomerQuery(
			customerId
		);
		if (customerDebit[0].debit) {
			if (customerDebit[0].debit > total) {
				...
				// eslint-disable-next-line no-plusplus
				for (let i = 0; i < returns.length; i++) {
					ReturnItem.updateDebitBalanceByIdQuery({
						itemReturnId: returns[i].iditem_return,
						debitBalance: 0,
					});
				}
