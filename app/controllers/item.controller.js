...
exports.getAllItems = async (req, res) => {
	try {
		pool.getConnection((err, connection) => {
			...
			connection.query(
				`select item.item_id, item.code, item.name, item.qty, item.length, item.description, item.isDeleted,
...
exports.addItem = async (item, res) => {
	// try{
	const { qty, length, subCategory_id, itemcode, itemname, itemdesc } =
		item.body;
	...
	pool.getConnection((err, connection) => {
		...
		connection.query(
			`insert into item (code, name, qty, length, description, subCategory_id) values
      ('${itemcode}', '${itemname}',${qty}, ${length}, '${itemdesc}',${subCategory_id});`,
			(err) => {
				connection.release();
				if (err) {
					res.status(500).send({
		...
		connection.query(
			...
			(batchErr, batchResult) => {
				...
				if (batchErr) {
					...
					connection.query(
						`insert into item_return_temp (cus_order_id, batch_id, name, date, quantity, length, cus_order_qty, order_status, cus_or_sup) values
            ('${custOrderId}', '${batchId}', '${name}', '${date}',${qty}, ${len}, ${cusOrderQty}, ${status}, '${cusOrSup}');`,
						(err, data) => {
							if (err) {
								...
								connection.query(
									`select * from item_return_temp ORDER BY item_return_id DESC LIMIT 1`,
									(err, data) => {
										if (err) {
											res.status(500).send({
												message: err,
...
exports.addReturnItemQuery = async (req, res) => {
	const { returnItemId, custOrderId, batchId, date, qty, status } = req.body;
