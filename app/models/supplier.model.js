...
Supplier.addSupplier = async ({ name, desc, phone, address }) => {
...
Supplier.getSupplierByName = async (supplierName) => {
	const result = await new Promise((resolve, reject) => {
		pool.getConnection((err, connection) => {
			...
			connection.query(
				`SELECT * FROM supplier WHERE name like '%${supplierName}%'`,
				(customerGetErr, customerGetResult) => {
					connection.release();
					if (customerGetErr) {
						reject(customerGetErr);
...
Supplier.addSupplyOrder = async (supplyOrder, res) => {
	const { date, supplireId, items } = supplyOrder;
	try {
		const result = await new Promise((resolve, reject) => {
			pool.getConnection((err, connection) => {
				if (err) {
				...
				connection.beginTransaction((err) => {
					connection.query(
						`INSERT INTO supplyorder( date, supplier_id)
              VALUES ('${date}', ${supplireId})`,
						(error, results) => {
							// console.log('******',results)
							if (error) {
							...
							for (let i = 0; i < items.length; i++) {
								if (!parseInt(items[i].qty)) {
									return connection.rollback(() => {
										resolve({
											status: 100,
											message: "item qty must in double",
										});
									});
								}
								connection.query(
									...
									(matchingBatchesError, matchingBatches) => {
										...
										if (matchingBatches.length > 0) {
											...
											connection.query(
												...
												[
													items[i].qty,
													items[i].supplyOrderNote,
													matchingBatches[0].batch_id,
												],
												(updateBatchError) => {
													if (updateBatchError) {
														return connection.rollback(() => {
