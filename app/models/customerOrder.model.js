	...
	const result = await new Promise((resolve, reject) => {
		pool.getConnection((err, connection) => {
			...
			connection.query(
				...
				(customerOrderErr, customerOrderResult) => {
					if (customerOrderErr) {
                  ...
                  `;
							...
							connection.query(
								sql,
								[customerOrderResult[i].idcustomer_order],
...
CustomerOrder.getCustomerOrdersByDates = async (
	...
	const result = await new Promise((resolve, reject) => {
		pool.getConnection((err, connection) => {
			if (err) throw err;
			connection.query(
				`SELECT * from customer_order where customer_idcustomer = '${customerId}' and 	customer_order_date >= '${fromDateF}' and customer_order_date <= '${toDateF}'`,
				(err, rows) => {
					if (err) {
						reject(err);
					} else {
